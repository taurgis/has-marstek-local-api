...
def test_devcontainer_runs_venus_a_145_and_149_mocks() -> None:
    """Compose must expose both Venus A solar encodings: legacy 145 and 149."""
    compose = (
        Path(__file__).resolve().parents[2] / ".devcontainer" / "docker-compose.yml"
    ).read_text(encoding="utf-8")

    assert '"--device", "VenusA", "--ver", "145"' in compose
    ...
    assert compose.count('"--ver", "145"') >= 2
